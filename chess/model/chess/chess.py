...
class Room(db.Model):
    ...
    members = db.Column(db.String(64))  # 格式为[[1001, 1], [1002, 2]] 第一个数字为uid, 第二个为状态，1为存在，2为不存在
    ...
    def get(cls, id_):
        ...
        # 处理头像
        row = User.query.join('wechat_user').filter(User.id.in_([data[0] for data in json.loads(room.members)])).with_entities(
    ...
    def join_or_leave_room(cls, room_id, member_id, join=True):
        ...
        #     members.append(member_id, 1)
        # elif member_id in members and not join:
        #     members.remove(member_id)
        # else:
        #     raise MemberInRoomNotFoundError
        if join:
            if [member_id, 1] in members:
                raise MemberInRoomAlreadyIn
            if [member_id, 0] in members:
                members[members.index([member_id, 0])] = [member_id, 1]
            else:
                members.append([member_id, 1])
        else:
            if [member_id, 0] in members:
                raise MemberInRoomNotFoundError
            if [member_id, 1] in members:
                members[members.index([member_id, 1])] = [member_id, 0]
            else:
                raise MemberInRoomNotFoundError
